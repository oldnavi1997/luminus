...
import { SHALOM_COST, getShippingCost } from "@/lib/shipping";
...
const shippingSchema = z.object({
  ...
  courier: z.enum(["shalom", "olva"], { error: "Selecciona un courier" }),
...
interface CheckoutFormProps {
  onShippingUpdate?: (data: { courier: "shalom" | "olva"; department: string }) => void;
}
...
export const CheckoutForm = forwardRef<CheckoutFormHandle, CheckoutFormProps>(
  function CheckoutForm({ onShippingUpdate }, ref) {
    ...
    function handleDepartmentChange(value: string) {
      ...
      setValue("postalCode", "");
      setDistrictValue("");
      if (courier) onShippingUpdate?.({ courier, department: value });
    }
    ...
    function handleCourierChange(value: "shalom" | "olva") {
      setValue("courier", value);
      onShippingUpdate?.({ courier: value, department: department ?? "" });
    }

    const olvaCost = department ? getShippingCost("olva", department) : null;
    ...
    return (
      <form className="space-y-5">
        ...
        {/* Método de envío */}
        <div className="bg-white border border-[#d5d5d5]/60 p-6 space-y-3">
          <p className="text-[11px] font-medium text-[#1e293b] uppercase tracking-[0.1em]">
            Método de envío
          </p>
          ...
          {/* Shalom */}
          <label
            className={`flex items-center justify-between gap-4 border px-4 py-3 cursor-pointer transition-colors duration-150 ${
              courier === "shalom"
                ? "border-[#c9a84c] bg-[#fdfaf3]"
                : "border-[#d5d5d5]/60 hover:border-[#c9a84c]/50"
            }`}
          >
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={() => handleCourierChange("shalom")}
                className="accent-[#c9a84c]"
              />
              <div>
                <p className="text-sm font-medium text-[#111111]">Shalom</p>
                <p className="text-[11px] text-[#111111]/40 mt-0.5">Nacional · 1–2 días hábiles</p>
              </div>
            </div>
            <span className="text-sm font-semibold text-[#111111]">S/ {SHALOM_COST.toFixed(2)}</span>
          </label>
          ...
          >
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={() => handleCourierChange("olva")}
                className="accent-[#c9a84c]"
              />
              <div>
                <p className="text-sm font-medium text-[#111111]">Olva Courier</p>
                <p className="text-[11px] text-[#111111]/40 mt-0.5">
                  {department
                    ? `${department} · 1–2 días hábiles`
                    : "Selecciona un departamento para ver el costo"}
                </p>
              </div>
            </div>
            <span className="text-sm font-semibold text-[#111111]">
              {olvaCost !== null ? `S/ ${olvaCost.toFixed(2)}` : "—"}
            </span>
          </label>
